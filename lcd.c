...
int r=0, w=0, queue[4096]={0};                       // 4096 Byte wr queue
...
void LCD_Write_Bus(int dat) {
   while (((w+1)%4096)==r) LCD_WR_Queue(); //If buffer full then spin...
   queue[w++]=dat;                        //...If/when not then store data...
   w%=4096;                                //...and advance write index!
}
...
void LCD_Address_QSet(u16 x1,u16 y1)
{
	LCD_WR_REG(0x2a);  // Column address setting
	LCD_WR_DATA(x1+lcd_conf.offset_x);
	LCD_WR_REG(0x2b);  // row address setting
...
{
	LCD_Address_QSet(x,y); // Set cursor position
	LCD_WR_DATA(color);
} 
